...
# SGWR settings: local geographic neighborhood, attribute similarity bandwidth,
# ridge stabilization, and minimum local sample size.
sgwr_params <- list(
  geo_k = 300L,
  h_attr = 1.0,
  ridge = 1e-6,
...
if (n < k_nn + 1L) {
  stop("Too few rows for k = ", k_nn, ".")
}

cat("Loaded ", n, " rows.\n", sep = "")

# Coordinates are assumed to be projected coordinates in EPSG:5070.
# If longitude and latitude are geographic coordinates, they should be projected before running this script.
pts <- st_as_sf(
  dat_raw,
...
# Construct k-nearest-neighbor spatial weights for geocomplexity calculation.
...
# Combine original variables and spatial pattern features.
dat_40d <- dat_raw %>%
  select(
    point_id,
...
# Compute pooled or fold-level evaluation metrics.
...
# Product-Gaussian similarity matrix used by PS and PSL.
...
# SGWR prediction for one fold.
# The local regression weight combines geographic proximity and attribute similarity.
...
# PS model:
# similarity-weighted averaging based on the expanded 40D feature space.
...
# PSL model:
# similarity is computed from 10D covariates;
# local RF is trained using the expanded 40D feature space.
...
# Merge predictions and calculate residuals.
res_all <- dat %>%
